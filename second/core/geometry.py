...
def points_in_convex_polygon_jit(points, polygon, clockwise=True):
    ...
    if clockwise:
        ...
        tmp=np.concatenate((np.array([num_points_of_polygon - 1]),np.arange(num_points_of_polygon-1)),axis=0)
        vec1 = polygon - polygon[:, tmp, :]
    else:
        # vec1 = polygon[:, [num_points_of_polygon - 1] +
        #                list(range(num_points_of_polygon - 1)), :] - polygon
        tmp = np.concatenate((np.array([num_points_of_polygon - 1]),
                              np.arange(num_points_of_polygon - 1)), axis=0)
        vec1 = polygon[:, tmp, :] - polygon
